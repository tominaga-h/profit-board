...
- [x] メンバー一覧・編集保存（追加/更新/削除混在）が動作する（ユーザー手動確認済み 2026-08-11）
- [x] email 付け替え（削除した行の email を別行で使う）が成功する（ユーザー手動確認済み 2026-08-11）
- [x] 実績のあるメンバー削除で FK 違反 → 409 → 既存の日本語メッセージが表示され、DB は保存前の状態のまま（全ロールバック）（ユーザー手動確認済み 2026-08-11）
...
> **実施記録（2026-08-11、Sonnet サブエージェントで実装・検収済み）:**
>
> - PUT はトランザクション内で DELETE → UPDATE（1行=1文ループ、バルク化禁止の理由コメント付き）
>   → INSERT。「変更行のみ送る」責務は既存どおりクライアント（`pickChanged`）に残した
> - has-costs はルートパラメータを `z.coerce.number()` で検証（ルートパラメータは常に文字列で
>   来るため coerce が必要。後続の projects でも同パターンを使うこと）
> - 「supabase-js は Tx を張れないので部分適用になる」系コメントを Tx 前提の記述に更新
> - **仕様差分（許容した独自判断）: 保存失敗メッセージをフェーズ別5種 → pgCode 別3種に統合。**
>   旧実装は削除/更新/追加のどこで失敗したかで別メッセージ（更新失敗は氏名入り）を出していたが、
>   単一 PUT + Tx ではどのフェーズで失敗したかをクライアントが区別できないため構造的に不可能。
>   FK 違反メッセージは旧文言を維持、UNIQUE 違反は「メールアドレスが重複しているため保存でき
>   ませんでした。」、その他は「メンバーの保存に失敗しました。」に統合
> - 検証: `make test` 218 件通過、`nuxi typecheck` エラーなし、3 エンドポイントとも未認証 curl 401
>
> **バグ修正（検収後に発覚・修正済み）: GET レスポンスの camelCase / snake_case 不一致。**
> Drizzle の `select()`（全列）はスキーマのプロパティ名 = camelCase（`familyName` 等）で返すが、
> クライアント型は `database.types.ts` 由来の snake_case（`family_name` 等）のため、
> `/members` 画面で全フィールドが undefined になった。fiscal-years は列名が `id` / `year` で
> 差が出ず発覚しなかった。**対策: GET 系エンドポイントは select で列を明示し snake_case の
> エイリアスで返す**（`members.get.ts` 参照）。**後続の projects / performance / 集計 /
> `/api/me` 差し替え（Task 7・9・10・11・12）すべてで同じ対応が必須。**
> なお `nuxi typecheck` はこの不一致を検出できない（`$fetch<Member[]>` は型表明であり
> 実レスポンスと照合されない）ため、画面確認が唯一の検出手段。
