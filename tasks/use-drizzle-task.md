...
- [ ] Supabase ダッシュボードで JWT Signing Keys（非対称鍵）が有効になっている
- [ ] 切替後も既存のログイン（Google OAuth）が正常に動作する
...
- [ ] ログイン済み登録ユーザーで `/api/me` が 200 と `AppUser` 相当を返す
- [ ] 未ログインで 401、登録済みでないアカウントで 403 を返す
- [ ] `user_metadata.email` を参照していない（改竄可能なため。プラン 5.1）
...
- [ ] `make test` 通過、`/api/me` が 3 パターンの応答を返す、既存画面は無影響
- [ ] ここで人間レビュー
