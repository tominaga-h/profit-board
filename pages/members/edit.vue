...
<template>
  <div>
    <PageHeader title="メンバー編集" subtitle="姓・名・メールアドレス・単価を一括編集します">
      <template #actions>
        <UButton color="gray" variant="solid" :disabled="isSaving" @click="handleCancel">
          キャンセル
        </UButton>
        <UButton
          icon="i-lucide-check"
    ...
    <div
      class="mb-4 flex items-center justify-between gap-4 rounded-xl border border-blue-100 bg-blue-50 px-4 py-3"
    >
      ...
      <template v-else-if="status !== FetchStatus.ERROR">
        <table v-if="drafts.length > 0" class="min-w-full">
          ...
          <tbody class="divide-y divide-slate-200">
            <tr v-for="draft in drafts" :key="draft.key" class="align-top">
              ...
              <td class="px-3 py-3">
                <input
                  v-model="draft.family_name"
                  type="text"
                  maxlength="50"
                  :class="inputClass(!!errorFor(draft).family_name)"
                  :aria-invalid="!!errorFor(draft).family_name"
                  aria-label="姓"
                />
              ...
              <td class="px-3 py-3">
                <input
                  ...
                  type="text"
                  maxlength="50"
                  :class="inputClass(!!errorFor(draft).first_name)"
                  :aria-invalid="!!errorFor(draft).first_name"
                  aria-label="名"
                />
              ...
              <td class="px-3 py-3">
                <input
                  ...
                  type="email"
                  maxlength="255"
                  :class="inputClass(!!errorFor(draft).email)"
                  :aria-invalid="!!errorFor(draft).email"
                  aria-label="メールアドレス"
                />
              ...
              <td class="px-3 py-3">
                <div class="relative">
                  ...
                  <input
                    ...
                    :aria-invalid="!!errorFor(draft).unit_price"
                    aria-label="単価"
                  />
                </div>
              ...
              <td class="px-6 py-3 text-right">
                <UButton
                  ...
                  :loading="checkingKey === draft.key"
                  :disabled="isSaving || !canDelete(draft) || checkingKey !== null"
                  :aria-label="`${draft.family_name} ${draft.first_name} を削除`"
                  :title="
                    canDelete(draft)
                      ? '削除する'
                      : 'ログイン中の自分自身は削除できません'
        ...
        <div class="px-6 py-4">
          <UButton
            ...
            icon="i-lucide-plus"
            :disabled="isSaving"
            class="border border-dashed border-blue-300"
            @click="addRow"
          >
            追加
          </UButton>
