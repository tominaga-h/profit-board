 ...
 * 行の検証エラー。列名 → メッセージ。
 *
 * インライン一括編集の画面（Task 6 のメンバー編集・Task 7 のプロジェクト編集）は
 * どれも「N行 × M列」の形をしていて、エラーは行と列の交点に出す必要がある。
 ...
 *
 * ★ 型引数だけがスキーマごとに変わり、処理は共通。Task 6 では
 *   toMemberRowErrors としてメンバー用に書いていたが、Task 7 で2例目
 *   （プロジェクト）が出たので、実体をここへ移してジェネリックにした。
 */
